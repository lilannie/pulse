	...
	obj.vote = (contractAddress, voterAddress, response) => {
		return new Promise(async (resolve, reject) => {
			...
			try {
				...
				//vote!
				result = contractInstance.changeVote.call(response, {from: voterAddress, gas: 4000000});
				contractInstance.changeVote(response, {from: voterAddress, gas: 4000000});

				//updates user history array
				//finds userHistory index by address
				//if vote is legitimate
				if (result) {
